 ...
 * acceleration of the espdrone (regardless of orientation - given by
 * AccWZ) is approaching -1 (AccWZ is 0 when espdrone is at rest). We
 ...
 *
 * An At Rest situation is considered identified when the espdrone is
 ...
 *
 * @param accX   Horizontal X acceleration (when espdrone is placed on its feet)
 * @param accY   Horizontal Y acceleration (when espdrone is placed on its feet)
 * @param accZ   Vertical Z acceleration (when espdrone is placed on its feet)
 */
void sitAwARTest(float accX, float accY, float accZ)
{
  ...
  if((fabs(accX) > SITAW_AR_THRESHOLD) || (fabs(accY) > SITAW_AR_THRESHOLD)) {
    /* If the X or Y accelerations are different than 0, the espdrone is not at rest. */
 ...
 * cut the thrust to the motors, avoiding the espdrone from running
