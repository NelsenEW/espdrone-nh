...
__ALIGN_BEGIN uint8_t  usbd_ed_CfgDesc[57] __ALIGN_END = {
...
static uint8_t  usbd_ed_Init        (void  *pdev, uint8_t cfgidx);
static uint8_t  usbd_ed_DeInit      (void  *pdev, uint8_t cfgidx);
static uint8_t  usbd_ed_DataIn      (void *pdev, uint8_t epnum);
static uint8_t  usbd_ed_DataOut     (void *pdev, uint8_t epnum);
static uint8_t  *usbd_ed_GetCfgDesc (uint8_t speed, uint16_t *length);
static uint8_t  usbd_ed_SOF         (void *pdev);
static uint8_t usbd_ed_Setup        (void *pdev , USB_SETUP_REQ  *req);

static USBPacket inPacket;
static USBPacket outPacket;

/* CDC interface class callbacks structure */
USBD_Class_cb_TypeDef ed_usb_cb =
{
  usbd_ed_Init,
  usbd_ed_DeInit,
  usbd_ed_Setup,
...
static uint8_t  usbd_ed_Init (void  *pdev,
                               uint8_t cfgidx)
{
  /* Open EP IN */
  DCD_EP_Open(pdev,
...
/**
  ...
  * @retval status
  */
static uint8_t  usbd_ed_DeInit (void  *pdev,
                                 uint8_t cfgidx)
{
  /* Open EP IN */
...
static uint8_t  *usbd_ed_GetCfgDesc (uint8_t speed, uint16_t *length)
{
  *length = sizeof (usbd_ed_CfgDesc);
  return usbd_ed_CfgDesc;
}
